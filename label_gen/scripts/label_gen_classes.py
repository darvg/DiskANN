...
FILTER_CLASSES = [
                    FilterClass(10, Uniform([0, 10], 'filter_dist', 10), 'size class',
                                Constant(2, 'size_dist'), (2, 2)),
                    FilterClass(12, LogNormal([0.5, -1, 3.8], 'filter_dist', 12), 'color class',
                                Uniform([2, 6], 'size_dist', 4), (2, 6)),
                    FilterClass(8, Uniform([0, 8], 'filter_dist', 8), 'format class',
                                Uniform([1, 3], 'size_dist', 2), (1, 3)),
                    FilterClass(10, Exponential([0, 0.8], 'filter_dist', 10), 'date class',
                                Uniform([3, 8], 'size_dist', 5), (3, 8)),
                    FilterClass(10, Exponential([0, 0.4], 'filter_dist', 10), 'usage rights class',
                                Constant(3, 'size_dist'), (3, 3))
                ]

# don't touch this
_TOTAL_FILTERS = sum([fc.size for fc in FILTER_CLASSES])
_PRIMES = []
...
#      Functions       #
########################
def _list_to_str(lst: List[List[int]], query_check: bool) -> str:
    '''
    Represents each list as a string:

    [a, b, c] -> "a,b,c" (for base points)
    [[a, b, c], [d, e, f]] -> "a|b|c&d|e|f" (for query points)
    '''

    s = []
    if query_check:
        for sublst in lst:
            s.append("|".join(map(str, sublst)))
        s = "&".join(s) + '\n'
    else:
        s = [i for sublst in lst for i in sublst]
        s = ",".join(map(str, s)) + '\n'
    return s
...
def build_parser() -> Dict[str, Any]:
    ...
    parser.add_argument('--query', action='store_true',
                        help="If present, generate labels for a set of queries. Otherwise, generate for a set of base points.")
    parser.add_argument('--unv', metavar='universal_label', type=int, default=-1, help="Universal label for base points.")
...
def generate_label_set(query_check: bool, universal_label: int) -> List[List[int]]:
    '''
    ...
    for fc in FILTER_CLASSES:
        ...
        if not query_check:
            if sampled_elements[0] == universal_label:
                sampled_elements = [sampled_elements[0]]
            elif universal_label in sampled_elements:
                sampled_elements = np.setdiff1d(sampled_elements, [universal_label])
...
def assign_label_sets(factor_hash: bool, powerset: bool, num_points: int, query_check: bool, universal_label: int) -> List[List[List[int]]]:
    '''
    ...
    for _ in range(num_points):
        ...
        if powerset:
            ...
            label_set = generate_label_set(query_check, universal_label)
        label_sets.append(label_set)
    return label_sets


def label_sets_to_file(label_sets: List[List[List[int]]], output_fname: str, query_check: bool):
    '''
    ...
    for s in label_sets:
        str_label_set = _list_to_str(s, query_check)
        output_file.write(str_label_set)
    output_file.close()


def classes_label_gen_pipeline(factor_hash: bool, output_fname: str,
                               powerset: bool, num_points: int, query_check: bool, universal_label: int):
    '''
    ...
    label_sets = assign_label_sets(factor_hash, powerset, num_points, query_check, universal_label)
    logger.info("Finished generating label sets")

    # 3.
    logger.info("Writing label sets to disk")
    label_sets_to_file(label_sets, output_fname, query_check)
    logger.info("Done")
...
if __name__ == '__main__':
    ...
    query_check = args['query']
    universal_label = args['unv']
    classes_label_gen_pipeline(factor_hash, out_fname, powerset, num_points, query_check, universal_label)
