...
typedef std::tuple<std::vector<std::vector<uint32_t>>, std::vector<uint32_t>>
    compute_base_points_per_query_return_values;

namespace diskann
{
DISKANN_DLLEXPORT std::vector<std::vector<label_set>> parse_query_label_file(path filename);

DISKANN_DLLEXPORT compute_base_points_per_query_return_values
compute_base_points_per_query(std::vector<std::vector<label_set>> query_pts_to_labels,
                              std::vector<label_set> base_pts_to_labels, std::string universal_label);
